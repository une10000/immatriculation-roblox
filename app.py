...
# --- CONFIGURATION PAGE ---
st.set_page_config(page_title="RCRP - Système Intégral v9.1", layout="wide")
            ...
            with st.container(border=True):
                ...
                if st.session_state.get(f"del_{idx}"):
                    with st.form(f"fd_{idx}"):
                        st.error("🚨 Supprimer définitivement ce véhicule ?")
                        vcd = st.text_input("🔑 Code secret pour confirmer", type="password")
                        if st.form_submit_button("🔥 CONFIRMER LA SUPPRESSION"):
                            if vcd == str(row['CODE']):
                                ...
                                st.success("🗑️ Véhicule supprimé"); time.sleep(1); st.rerun()
...
with tabs[1]:
    ...
    with st.expander("👤 Créer un nouveau dossier conducteur"):
        with st.form("f_pts_v91"):
            ...
            if st.form_submit_button("💾 Créer le dossier"):
                if c_a == CODE_ADMIN_GENERAL:
                    ...
                    st.success("✅ Dossier créé et 15 000$ ajoutés à la banque !"); time.sleep(1); st.rerun()
            ...
            with st.container(border=True):
                ...
                with st.expander("⚙️ Modifier les points"):
                    with st.form(f"fp_{idx}"):
                        nv = st.number_input("Nouveau solde de points", 0, 25, int(row['PTS']))
...
with tabs[2]:
    ...
    if not df_bank.empty and sb:
        ...
        for idx, row in res_b.iterrows():
            ...
            with st.expander("💸 Opération (Retrait/Ajout/RCT)"):
                with st.form(f"fb_v91_{idx}"):
                    code = st.text_input("🔑 Code Autorisation", type="password"); mnt = st.number_input("💵 Montant", step=100.0)
                    ...
                    if c_ret.form_submit_button("📉 RETIRER / FACTURER"):
                        if code == CODE_ENTREPRISE:
                            ...
                            if mask.any():
                                ...
                                st.success(f"💸 Facture RCT payée ! {mnt}$ transférés à {MON_PSEUDO_ROBLOX}"); time.sleep(1); st.rerun()
                        elif code == CODE_ADMIN_GENERAL:
                            df_bank.at[idx, "Solde"] = solde - mnt
                            conn.update(worksheet="Banque", data=df_bank)
                            st.success("📉 Retrait/Amende effectué !"); time.sleep(1); st.rerun()
                    if c_aj.form_submit_button("📈 AJOUTER (Police/Staff)"):
                        if code == CODE_ADMIN_GENERAL:
                            df_bank.at[idx, "Solde"] = solde + mnt
                            conn.update(worksheet="Banque", data=df_bank)
                            st.success("📈 Somme ajoutée au compte !"); time.sleep(1); st.rerun()

# ==========================================
# 📜 ONGLET 4 : LOGS
# ==========================================
with tabs[3]:
    if st.text_input("🔑 Accès aux Logs", type="password") == CODE_ADMIN_GENERAL:
