...
# - Création Profil : Date d'arrivée générée AUTOMATIQUEMENT.
# - Nouveauté : Les civils peuvent immatriculer et gérer leurs véhicules (Code Secret).
# ==============================================================================
...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="RCRP - Système Intégré", page_icon="🏛️", layout="wide")
...
st.markdown("""
    ...
    .id-card { background: rgba(52, 152, 219, 0.1); border: 1px solid #3498db; border-radius: 15px; padding: 20px; }
    .license-box { padding: 20px; border-radius: 15px; text-align: center; background: rgba(0,0,0,0.3); }
    </style>
...
ASSET_LOGO = "https://i.imgur.com/8K5S7f6.png" # Lien de secours si le Discord expire
...
def load_table(name):
    st.cache_data.clear()
    try: return conn.read(worksheet=name, ttl=0).dropna(how='all').fillna("")
    except: return pd.DataFrame()

def commit_log(agent, category, info):
    try:
        logs = load_table("Logs")
        entry = pd.DataFrame([{"Horodateur": datetime.now().strftime("%d/%m/%Y %H:%M"), "Opérateur": agent, "Catégorie": category, "Description": info}])
...
if st.session_state.role is None:
    ...
    with col_c:
        if st.button("👤 Entrer (Espace Civil)", use_container_width=True):
...
# --- MODULE IMMATRICULATION (LOGIQUE COMMUNE AMÉLIORÉE) ---
...
# (Le reste du code pour la Banque, le Permis et le Staff reste identique au précédent)
# ==============================================================================
