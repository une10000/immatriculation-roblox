...
# ==========================================
# ONGLET 2 : POINTS DE PERMIS (FIXED)
# ==========================================
with tabs[1]:
            ...
            if st.form_submit_button("Créer le dossier"):
                if admin_code == CODE_ADMIN_GENERAL:
                    if new_roblox:
                        ...
                        # 2. Créer Banque (Fix de l'erreur AttributeError)
                        ...
                        st.success("✅ Dossier et compte bancaire créés !")
                        time.sleep(1); st.rerun()
    ...
    if not df_pts.empty and search_p:
        ...
        for idx, row in res.iterrows():
            ...
            with col_m:
                with st.expander("⚙️ Gérer Points"):
                    with st.form(key=f"mod_pts_{idx}"):
                        ...
                        if st.form_submit_button("Valider"):
                            if ac == CODE_ADMIN_GENERAL:
                                ...
                                log_action(f"Points mis à {n_p}", row.get('Nom Roblox'))
                                st.rerun()
            with col_d:
                if st.button("🗑️ Supprimer", key=f"pre_del_{idx}"):
                    st.session_state[f"del_{idx}"] = True
                if st.session_state.get(f"del_{idx}", False):
                    dc = st.text_input("Code de confirmation", type="password", key=f"c_{idx}")
                    if st.button("🔥 CONFIRMER", key=f"real_{idx}") and dc == CODE_ADMIN_GENERAL:
...
with tabs[2]:
    ...
    st.info("ℹ️ Les comptes sont créés automatiquement via l'onglet Permis.")
    sb = st.text_input("🔍 Chercher un compte").strip()
            ...
            with st.expander("🛡️ Transaction Admin"):
                with st.form(key=f"b_tr_{idx}"):
                    ...
                    if b1.form_submit_button("📉 Retirer") and abc == CODE_ADMIN_GENERAL:
                        ...
                        log_action(f"Retrait (-{mnt})", row.get('Nom Roblox'))
                        st.rerun()
                    if b2.form_submit_button("📈 Ajouter") and abc == CODE_ADMIN_GENERAL:
                        df_bank.at[idx, "Solde"] = solde + mnt
                        conn.update(worksheet="Banque", data=df_bank)
                        log_action(f"Ajout (+{mnt})", row.get('Nom Roblox'))
                        st.rerun()
...
with tabs[3]:
    ...
    except: st.warning("Feuille 'Logs' introuvable.")

st.markdown("<div style='position: fixed; left: 10px; bottom: 10px; color: grey; font-size: 12px;'>Version v4.6 - Bug Fix & Auto-Bank</div>", unsafe_allow_html=True)
