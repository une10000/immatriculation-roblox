        ...
        with st.container(border=True):
            ...
            if agent_code_saisi:
                ...
                if not res_agent.empty:
                    ...
                    if not en_service:
                        # Texte d'information Hors-Service
                        st.info(f"💡 **Information :** Vous êtes actuellement **Hors-Service**. Pour commencer à comptabiliser votre temps de paie et accéder aux outils d'intervention, veuillez cliquer sur le bouton **Début de Service** ci-dessous.")
                        ...
                        # Texte d'information En-Service
                        st.warning(f"🚨 **Information :** Vous êtes en service actif (**{job_auto}**). N'oubliez pas de cliquer sur **Fin de Service** avant de quitter pour que vos minutes soient bien enregistrées dans la base de données des paies.")
...
# --- 2. RECHERCHE & MANDATS ---
        ...
        if target == "---":
            ...
            with col_db:
                ...
                if not t_veh.empty:
                    for _, v in t_veh.iterrows():
                        a_stat = str(v['Assurance']).upper()
                        clr = "#27ae60" if "RCT" in a_stat else "#e74c3c"
                        st.markdown(f"""<div style="border-left:4px solid {clr}; padding:5px; margin-bottom:5px; background:#262730;">
                        <b>{v['Numéro de la plaque']}</b> ({v['Marque du véhicule']})<br>
                        <small>Assu: {a_stat}</small></div>""", unsafe_allow_html=True)
                
                st.markdown("🕒 **DERNIERS FAITS**")
                try:
                    df_all_f = cloud_conn.read(worksheet="Factures").fillna("")
                    for _, h in df_all_f[df_all_f["Cible"] == target].tail(2).iterrows():
                        st.caption(f"• {h['Date_Emission']} : {h['Montant']}$ ({h['Statut']})")
                except: pass

    else: st.error("Accès non autorisé.")
