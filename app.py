...
# --- ONGLET 2 : DOSSIERS CITOYENS (CORRIGÉ AVEC DOUBLE ÉCRITURE) ---
with tab_dossier:
    st.header("🪪 Dossiers Administratifs")
    if st.session_state.role == "Staff":
        with st.container(border=True):
            st.subheader("🧧 Console de Création")
            with st.form("new_cit_form"):
                ...
                if st.form_submit_button("Créer le Dossier Complet (15k + 25 pts)"):
                    if n_rob and n_dis:
                        d_actuelle = datetime.now().strftime("%d/%m/%Y")
                        
                        # Données pour la Banque
                        new_b = pd.DataFrame([{"Solde": 15000, "Nom Discord": n_dis, "Nom Roblox": n_rob, "Date d'arrivée": d_actuelle, "Emploiement": n_job}])
                        # Données pour les Permis
                        ...
                        st.success(f"✅ Citoyen créé : 15,000$ versés et 25 points attribués.")
