...
# --- ONGLET 1 : IMMATRICULATIONS (CALCUL LIVE SANS EFFACER LE CODE) ---
with tabs[0]:
    st.header("🚗 Registre des Véhicules")
    
    with st.expander("➕ Enregistrer un véhicule", expanded=True):
        # Utilisation de widgets hors formulaire pour forcer l'actualisation du calcul RCT/AVERIS
        ...
        code_v = st.text_input("🔑 Code Secret du véhicule", type="password")
        
        # Logique de calcul (S'actualise dès qu'on change le dropdown)
        cost_ville = 175
        cost_assu = 0
        if "AVERIS" in assu_select: cost_assu = 130
        elif "RCT" in assu_select: cost_assu = 150
        ...
        if "RCT" in assu_select and rct_count >= 2:
            cost_assu = 0
            st.success("🎁 Offre Trio : Assurance offerte pour ce véhicule !")
        ...
        if st.button("✅ Valider et Payer l'enregistrement"):
            if user_select != "---" and plaque_v and code_v:
                ...
                if current_solde >= total_facture:
                    ...
                    # Virement aux entreprises
                    dest_user = None
                    if "AVERIS" in assu_select: dest_user = TARGET_AVERIS
                    elif "RCT" in assu_select: dest_user = TARGET_RCT
                    
                    if dest_user and cost_assu > 0:
                        idx_dest = df_banque[df_banque["Nom Roblox"] == dest_user].index[0]
                    ...
                    st.success("Immatriculation réussie !"); time.sleep(1); st.rerun()
                else:
